...
public class Lexer  {
	...
	private Token getNextToken() {
		skipWhitespaces();
		switch (actChar) {
			case -1: return new TokenEOF(pos,line);
		...
		if (Character.isDigit((char) actChar)) return lexNum();
	...
	private Token lexName() {
		...
		while(!Character.isSpace((char) lookAhead) && lookAhead != -1 && lookAhead != 34 && !(lookAhead >= 40 && lookAhead <=47) 
	...
	private int getNextChar() {
		...
		charAt = (int) code.charAt(sInd);

		if (oldLookahead != -1) {
			charAt=oldLookahead;
		}else{
			if (sInd >= code.length()) charAt = -1;
			else {
				charAt=(int) code.charAt(sInd);
				sInd++;
			}
		...
		if (sInd >= code.length()) lookAhead = -1;
		else {
